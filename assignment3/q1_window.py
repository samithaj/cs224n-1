...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    ### YOUR CODE HERE (5-20 lines)
        for idx in range(len(sentence)):
            prev_window = [sentence[i] if i >= 0 else start for i in range(idx-window_size, idx)]
            next_window = [sentence[i] if i < len(sentence) else end for i in range(idx+1, window_size+idx+1)]
            window = [feature for features in prev_window for feature in features] + \
                     sentence[idx] + \
                     [feature for features in next_window for feature in features]
