...
def cbow(currentWord, C, contextWords, tokens, inputVectors, outputVectors,
    ...
    predicted = inputVectors[context_indices, :].sum(axis = 0).reshape(-1, 1)

    cost, gradInvhat, gradOut = word2vecCostAndGradient(predicted, target_idx, outputVectors, dataset)
    gradInvhat = gradInvhat.flatten()

    for i in xrange(len(context_indices)):
        gradIn[context_indices[i], :] += gradInvhat
